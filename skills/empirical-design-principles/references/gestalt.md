...
**Not supported by the research: the negative-space folklore — that hidden figures in negative space convey meaning unconsciously.** Ke, Gupta, Lo, Ting & Tseng (2023) ran a Posner orienting paradigm across North American and Taiwanese samples — four numbered experiments, five participant samples (1, 2, 3, 4a, 4b) — and found no cue congruency effect until the arrow was **both** highlighted blue **and** the competing change-detection task removed (Experiment 4b), concluding that people do not unconsciously perceive the FedEx arrow to the point of producing an attentional effect. Do not repeat the shorter gloss "unless the arrow was explicitly highlighted", which is the authors' own abstract compression and is contradicted by their Experiment 3: that experiment painted every arrow blue and still found no effect — "despite the fact that all FedEx arrows have been highlighted blue". Their conclusion is explicit that "these two conditions need to both be present". That is the one study that tested the claim directly.
...
**Limits** — The critical limit for interfaces. Mary A. Peterson & Elizabeth Salvagio, "Inhibitory competition in figure–ground perception: context and convexity", *Journal of Vision* 8, no. 16 (15 December 2008), Article 4, found that for a *single* edge "there is only a weak bias toward seeing the figure on the convex side" — roughly 57% of trials in two-region displays, rising to ~89% with eight alternating regions, and to ~89% only when the concave regions were homogeneous in colour. Wagemans et al. (2012, p. 1201): "when studied in more controlled experiments, the classic configural principles turn out weaker than previously supposed."

**Read the exposure condition before carrying the 57% into a UI argument.** Peterson & Salvagio cut exposure to **100 ms** in the same step as they varied region count — per Wagemans et al. (2012), the displays contained two to eight regions and "in addition, the exposure duration was reduced to 100 ms". So 57% is the two-region rate *at 100 ms*; the two-region rate under unlimited viewing is not established by this paper. The classic demonstrations differ on both variables at once, region count *and* exposure, and the long-exposure figure sitting in this file's own cited review is the opposite end: regions with convex parts "were seen as figure on approximately **90%** of trials". Region number was also a between-subject factor (Goldreich & Peterson, 2012, n. 2 — "across-subject trends, not within-subject trends").

An interface is looked at for seconds, not for 100 ms, so **neither figure is a UI figure**: 57% is measured under an exposure a UI never imposes, and Wagemans hedges the long-exposure direction too ("it is not clear that reports reflected initial organization"). What survives is the weaker and more useful claim — convexity is a real but modest cue whose strength is contingent on context and viewing conditions that no interface reproduces, so do not cite a percentage for it at all. Do not write "a UI is the two-region case, which is where the effect is weakest"; that conclusion needs the 100 ms number to hold at seconds, and it has not been shown to.
...
**Predicts** — An enclosing contour overrides both proximity and similarity in Palmer's demonstrations (Figs. 2C, 2D). Dashed and even illusory contours suffice (Figs. 2E, 2F). Common region operates *after* stereoscopic depth perception: Palmer's Fig. 3 transparency demonstration shows dots grouping with the coplanar enclosing ellipses, not the retinally nearer ones. Palmer reports a preference for the smallest enclosing region, with hierarchical nesting — but state it as the preference he states it as, not as a rule: his abstract says "dominated by the smallest background area" while his body (p. 442) says only that "a noticeable preference is observed in favor of the smaller contours", and immediately adds that "Quantitative experimental procedures will be required to assess the relative dominance of these different properties, however, for spontaneous impressions are variable and difficult to assess."
...
- Edgar Rubin, *Synsoplevede Figurer* (Copenhagen: Gyldendalske Boghandel, 1915), German edition *Visuell wahrgenommene Figuren: Studien in psychologischer Analyse* (Copenhagen: Gyldendalske Boghandel, 1921). Rubin's non-membership of and scepticism toward the Gestalt school is documented in Jörgen L. Pind, *Edgar Rubin and Psychology in Denmark: Figure and Ground* (Springer, 2014).
...
- Mary A. Peterson & Elizabeth Salvagio, "Inhibitory competition in figure-ground perception: context and convexity", *Journal of Vision* 8, no. 16 (15 December 2008), Article 4, doi:10.1167/8.16.4. The abstract carries the quoted sentence but no percentages, and the JOV full text is paywalled to us; the 57% and 89% figures and the 100 ms exposure were recovered from Wagemans et al. (2012, PMC3482144) and cross-checked against Brooks, Peterson et al., *Neuropsychologia* 50 (2012), PMC3405515, which independently reports "only 57% choice of the convex region for bipartite displays".
- Daniel Goldreich & Mary A. Peterson, "A Bayesian observer replicates convexity context effects in figure-ground perception", *Seeing and Perceiving* 25, no. 3-4 (2012): 365–395. Note 2 records that region number was manipulated between subjects, so the 57%-to-89% rise is an across-subject trend.
