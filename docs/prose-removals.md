...
| logmind | 43 | Correcting three things this skill taught that are now false, on a file with **52 bytes** of headroom — so the corrections had to be paid for. The 43 words are: (a) the clause *"branch edits cause exactly the cross-PR conflicts this design eliminates"*, which restates the section's own heading (*"Derived docs are main-only — don't edit them on a branch"*) and its opening paragraph; (b) one word from *"straight from source"*; (c) tightening of the three passages being corrected, which this gate rightly counts as removal. **Nothing was cut to avoid saying something.** The corrections themselves added 58 words — a net gain the gate correctly refused to be satisfied by, which is #258's churn detection doing exactly its job on the day it shipped. Final headroom: 8 bytes. |
