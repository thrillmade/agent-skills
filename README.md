...
| [`night-mode`](skills/night-mode/SKILL.md) | Policy for a session a human explicitly handed over to run **unattended** — the handover contract, the reversible-and-invisible boundary on overnight action, "a scheduled wake is never consent", named hard stops, and the morning digest. Layers on `session-heartbeat`. Not dark mode. |
